    ...
    <div id="myTabContent" class="tab-content">
        <div class="tab-pane fade in active" id="home">
            <div v-if="form.parent != 0" class="form-group" style="margin-top: 15px;">
                ...
                <div class="col-md-4">
                    <input type="text" class="form-control" value="<?php echo ($parentName); ?>(<?php echo ($parentMark); ?>)" disabled>
                </div>
            </div>
            <div class="form-group" style="margin-top: 15px;">
                ...
                <div class="col-md-10">
                    <select v-model="form.tid" style="margin-top: 8px;" v-on:change="tpl">
                        <option value="0" selected>请选择栏目类型</option>
            ...
            <div class="form-group" style="margin-top: 15px;">
                <label class="col-md-2 control-label">发布语言:</label>
                <div class="col-md-10">
                    <select v-model="form.lang" style="margin-top: 8px;" >
                        <option value="-1" selected>请选择语言类型</option>
    ...
    <script>  
        var form=new Vue({
            ...
            data:{
                ...
                cateType:<?php echo ($cateType); ?>,
                tpllist:<?php echo ($tplList); ?>,
                tplContent:<?php echo ($tplContent); ?>,
                form:{
                    name:"",
                    tid:"",
                    hide:"1",
            ...
            methods:{
                post:function(event){
                    if(form.form.name==""){
                        dialog.error("栏目名称不能为空");
                        return ;
                    }
                    if(form.form.tid==0){
                        dialog.error("请选择栏目类型");
                        return ;
                    }
                    if(form.form.mark==""){
                        dialog.error("栏目标识不能为空");
                        return ;
                    }
                    form.form.content=editor.html();
                    $.post("?m=Admin&c=Cate&a=doCateAdd",form.form,function(res){
                        if(res.status==1){
                            dialog.toconfirm(res.message);
                            dialog.confirm("是否继续添加","是","form.form.name='';form.form.tid=0;form.form.hide=1;form.form.order1=10;form.form.mark='';","否","nav.cateList();");
                        }
                        else{
